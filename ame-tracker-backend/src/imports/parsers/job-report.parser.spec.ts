...
describe('parseJobReport', () => {
  it('maps Item / Alpha # columns from a T4 item schedule', async () => {
    ...
    expect(result.rows[0].alphaNumber).toBe('1')
    ...
    expect(result.rows[1].alphaNumber).toBe('3-')
  ...
  it('parses the sample 637.xlsx item schedule', async () => {
    ...
    expect(result.rows[0].fitting).toBe('Cut Duct')
    expect(result.rows.find((r) => r.alphaNumber === '3-')?.fitting).toBe('Cut Duct')
    expect(result.rows.find((r) => r.alphaNumber === '3-')?.pieceNumber).toBe('3')
    expect(result.rows.find((r) => r.alphaNumber === '3')?.quantity).toBe(4)
    expect(result.rows.find((r) => r.alphaNumber === '17A')?.pieceNumber).toBe('17')
